...
*
* Credit goes to coolAlias for this class!
*/

public abstract class NetworkMsgHandler<T extends IMessage> implements IMessageHandler <T, IMessage>
{
	/**
	...
	*/
	...
	public IMessage onMessage(T message, MessageContext ctx)
	{
		...
		if (ctx.side.isClient())
		{
			...
			return handleClientMessage(ModCore.proxy.getPlayerEntity(ctx), message, ctx);
		}
		else
		{
			// server side proxy will return the server side EntityPlayer
			return handleServerMessage(ModCore.proxy.getPlayerEntity(ctx), message, ctx);
		}
	}
}
